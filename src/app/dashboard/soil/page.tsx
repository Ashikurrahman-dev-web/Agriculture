...
const translations = {
  ...
  bn: {
    ...
    characteristics: "দেখা যাওয়া বৈশিষ্ট্যসমূহ",
    suitableCrops: "উপযুক্ত ফসলসমূহ",
    soilImprovements: "মাটি উন্নতির উপায়",
    ...
    historyTitle: "পরীক্ষার ইতিহাস",
    noHistory: "এখনো কোনো ইতিহাস পাওয়া যায়নি।",
...
export default function SoilDoctorPage() {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const res = await fetch(`${serverUrl}/api/soil/history`);
      if (res.ok) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.error("AI soil analysis error:", err);
...
className="mb-6 flex items-center gap-2 text-sm font-semibold text-[#1F3D2B] transition hover:text-[#2F5943]"
      >
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          {/* LEFT & CENTER PANEL: Main Analyzer Section (2 Columns) */}
          ...
          <div className="sticky top-8 rounded-3xl border border-[#E4DFD1] bg-white p-6 shadow-sm">
              <div className="mb-4 flex items-center justify-between border-b border-[#FAF8F3] pb-4">
                ...
                <span className="rounded-full bg-[#EAF0E8] px-2.5 py-0.5 text-xs font-bold text-[#1F3D2B]">
                  {history.length}
                </span>
              </div>
              ...
              {loadingHistory ? (
                <div className="flex items-center justify-center py-8">
                  <Loader2 className="animate-spin text-[#1F3D2B]" size={20} />
                </div>
              ) : history.length === 0 ? (
                <p className="py-8 text-center text-xs text-[#6B7A6E]">
                  {t.noHistory}
                </p>
              ) : (
                <div className="max-h-[600px] space-y-3 overflow-y-auto pr-1">
                  {history.map((item, idx) => (
                    <div
                      ...
                      className={`group flex cursor-pointer items-center justify-between rounded-2xl border p-3.5 transition ${
                        result?.soilType === item.soilType
                          ? "border-[#1F3D2B] bg-[#EAF0E8]/30"
                          : "border-[#E4DFD1]/60 bg-[#FAF8F3]/50 hover:border-[#1F3D2B]/40 hover:bg-[#EAF0E8]/20"
                      }`}
                    >
